...
const BlogDetail = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        console.error("Error while fetching blog details:", err);
        setError("An error occurred while loading blog details. Please try again later.");
        setLoading(false);
  ...
  const getImageUrl = (blogItem) => {
    ...
    if (blogItem.media) {
      ...
      return `${process.env.REACT_APP_API_URL || 'http://localhost:8080'}${blogItem.media}`;
    }
