...
public interface ForumCommentRepository extends JpaRepository<ForumComment, Integer> {
    ...
    @Query("SELECT COUNT(c) FROM ForumComment c WHERE c.createdBy = :userId")
    long countByCreatedBy(@Param("userId") int userId);
    
    @Query("SELECT c.forumPostId FROM ForumComment c WHERE c.createdBy = :userId")
