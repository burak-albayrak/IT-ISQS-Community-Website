...
public class KNNRecommendationService {
    ...
    private List<ForumPostFeatureVector> convertPostsToFeatureVectors() {
        ...
        return forumPostRepository.findAllWithCategoryAndCounts().stream()
            .map(post -> {
                ...
                int categoryIndex = allCategories.indexOf(post.getCategory().getCategoryId());
                if (categoryIndex >= 0) {
    ...
    private List<RecommendationResponse> getFallbackRecommendations() {
        ...
        return forumPostRepository.findTopNByOrderByLikesCountDescCommentCountDesc(K_NEIGHBORS)
            .stream()
            .map(post -> new RecommendationResponse(
                post.getForumPostID(),
                post.getTitle(),
                post.getCategory().getCategoryId(),
