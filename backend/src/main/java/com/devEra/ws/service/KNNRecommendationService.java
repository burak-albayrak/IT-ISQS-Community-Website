...
public class KNNRecommendationService {
    ...
    private List<ForumPostFeatureVector> convertPostsToFeatureVectors() {
        ...
        return forumPostRepository.findAllWithCategoryAndCounts().stream()
            .map(post -> {
                ...
                ForumCategory firstCategory = post.getCategories().isEmpty() ? null : post.getCategories().get(0);
                int categoryIndex = firstCategory != null ? allCategories.indexOf(firstCategory.getCategoryId()) : -1;
                if (categoryIndex >= 0) {
                ...
                return new ForumPostFeatureVector(
                    post.getForumPostID(),
                    features,
                    firstCategory != null ? firstCategory.getCategoryId() : 0,
                    post.getLikesCount(),
    ...
    private List<RecommendationResponse> getFallbackRecommendations() {
        ...
        return forumPostRepository.findAllOrderByLikesAndCommentsDesc().stream()
            .limit(K_NEIGHBORS)
            .map(post -> {
                // İlk kategoriyi al (varsa)
                ForumCategory firstCategory = post.getCategories().isEmpty() ? null : post.getCategories().get(0);
                return new RecommendationResponse(
                    post.getForumPostID(),
                    post.getTitle(),
                    firstCategory != null ? firstCategory.getCategoryId() : 0,
                    firstCategory != null ? firstCategory.getName() : "Uncategorized",
                    post.getLikesCount(),
                    post.getCommentCount(),
